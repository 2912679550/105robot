...
void Image2RTSPNodelet::onInit() {
    ...
    string pipeline_tail =  " name=pay0 pt=26 )"; // Gets completed based on rosparams below
    ...
    for(XmlRpc::XmlRpcValue::ValueStruct::const_iterator it = streams.begin(); it != streams.end(); ++it)
    {
        ...
        if (stream["type"]=="cam")
        {
            pipeline = "( " + static_cast<std::string>(stream["source"]) + " ! jpegparse ! rtpjpegpay" + pipeline_tail;

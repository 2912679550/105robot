...
echo  'KERNEL=="ttyUSB0", ATTRS{idVendor}=="10c4", ATTRS{idProduct}=="ea60",ATTRS{serial}=="0011", MODE:="0777", GROUP:="dialout", SYMLINK+="imu_front"' >/etc/udev/rules.d/wheeltec_fdi_imu_gnss.rules
# echo  'KERNEL=="ttyUSB*", ATTRS{idVendor}=="10c4", ATTRS{idProduct}=="ea60",ATTRS{serial}=="0013", MODE:="0777", GROUP:="dialout", SYMLINK+="imu_back"' >/etc/udev/rules.d/wheeltec_fdi_imu_gnss.rules
